...
public final class OrderBrushOrderReversed {
    ...
    private final class ShopInfo {
        ...
        // recent transactions
        PriorityQueue<Record> recentRecords = new PriorityQueue<>(Record.TIME_COMPARATOR_REVERSED);
        PriorityQueue<Record> recentOneHour = new PriorityQueue<>(Record.TIME_COMPARATOR_REVERSED);
        ...
        public final void update(Record record) {
            ...
            if (!info.isPreviousBrushOrder) {
                while (!info.recentRecords.isEmpty()
                        && info.recentRecords.peek().eventTime.compareTo(oneHourFuture) > 0) {
                    info.recentRecords.remove();
                }
            }

            // remove old transactions from recentOneHour
            while (!info.recentOneHour.isEmpty() && info.recentOneHour.peek().eventTime.compareTo(oneHourFuture) > 0) {
                info.recentOneHour.remove();
            }

            // record suspicious activity
            if (concentration(info.recentOneHour) >= 3) {
